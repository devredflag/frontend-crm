...
import {
  ...
  Zap, Globe, ExternalLink, Info, Save, BarChart3,
  ChevronDown, LayoutDashboard, Building2, Users,
  Search, ClipboardList, Calendar, RefreshCw
} from "lucide-react";
...
const css = `
  ...
  * { font-family: 'Plus Jakarta Sans', sans-serif; box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .channel-card:hover { border-color:rgba(41,128,185,0.3); background:rgba(255,255,255,0.85); transform:translateY(-2px); box-shadow:0 8px 24px rgba(41,128,185,0.1); }
  .channel-card.selected { border-color:#2980b9; background:rgba(41,128,185,0.05); box-shadow:0 0 0 3px rgba(41,128,185,0.1); }
...
export default function Perfil() {
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem("crm_comm_prefs");
    if (saved) {
      try {
  ...
  const handleSave = () => {
    localStorage.setItem("crm_comm_prefs", JSON.stringify({ emailProvider, whatsappMode }));
    setSaved(true);
  ...
  return (
    <div style={{ display:"flex", height:"100vh", overflow:"hidden", position:"relative" }}>
      ...
      <div style={{ width:220, flexShrink:0, height:"100vh", overflowY:"auto", position:"relative", zIndex:10, background:"linear-gradient(180deg,#1a3a5c 0%,#0f2a44 60%,#0a1f33 100%)", boxShadow:"4px 0 24px rgba(0,0,0,0.18)", display:"flex", flexDirection:"column", padding:"0 12px 20px" }}>
        ...
        {/* Avatar do usuário — agora com active highlight já que estamos na página de perfil */}
                ...
                <motion.div key="comunicacao" initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-6 }} transition={{ duration:0.25 }}>
                  ...
                  <motion.div className="glass-card" style={{ padding:"20px 22px", marginBottom:16 }} initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.05 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:16 }}>
                      ...
                      <div>
                        <div style={{ fontSize:14, fontWeight:700, color:"#0f2133" }}>E-mail</div>
                        <div style={{ fontSize:11, color:"rgba(20,45,70,0.45)" }}>Selecione o cliente de e-mail padrão</div>
                      </div>
                    </div>
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                      ...
                      {/* Gmail */}
                      <div className={`channel-card${emailProvider === "gmail" ? " selected" : ""}`} onClick={() => setEmailProvider(emailProvider === "gmail" ? null : "gmail")}>
                        {emailProvider === "gmail" && (
                          <div style={{ position:"absolute", top:10, right:10, width:20, height:20, borderRadius:"50%", background:"#2980b9", display:"flex", alignItems:"center", justifyContent:"center" }}>
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:10 }}>
                          ...
                          <svg width="32" height="32" viewBox="0 0 48 48">
                            <path fill="#EA4335" d="M24 24L4 10v28h6V21l14 10 14-10v17h6V10z"/>
                            <path fill="#FBBC05" d="M4 10l20 14L44 10H4z"/>
                            <path fill="#34A853" d="M38 38V21l6-5v22z"/>
                            <path fill="#4285F4" d="M4 38V21L10 16v22z"/>
                          </svg>
                          <div>
                            <div style={{ fontSize:14, fontWeight:700, color:"#0f2133" }}>Gmail</div>
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:5, fontSize:11, color:"rgba(20,45,70,0.5)" }}>
                          <Globe style={{ width:11, height:11, marginTop:1 }}/>
                          <span>mail.google.com</span>
                        </div>
                        <div style={{ marginTop:8, padding:"6px 10px", borderRadius:8, background:"rgba(234,67,53,0.06)", border:"1px solid rgba(234,67,53,0.12)", fontSize:10, color:"#c0392b", fontWeight:600 }}>
                          Recomendado para contas Google Workspace
                        </div>
                      </div>

                      {/* Outlook */}
                      <div className={`channel-card${emailProvider === "outlook" ? " selected-outlook" : ""}`} onClick={() => setEmailProvider(emailProvider === "outlook" ? null : "outlook")}>
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:10 }}>
                          ...
                          <svg width="32" height="32" viewBox="0 0 48 48">
                            ...
                            <path fill="#fff" d="M26 12h14v24H26z"/>
                            <path fill="#0078d4" d="M28 14h10v4H28zm0 6h10v4H28zm0 6h10v4H28z"/>
                            <ellipse cx="16" cy="24" rx="8" ry="10" fill="#fff"/>
                            <ellipse cx="16" cy="24" rx="5" ry="7" fill="#0078d4"/>
                          </svg>
                          <div>
                            <div style={{ fontSize:14, fontWeight:700, color:"#0f2133" }}>Outlook</div>
                            <div style={{ fontSize:10, color:"rgba(20,45,70,0.45)" }}>Web ou app instalado</div>
                          </div>
                        </div>
                        <div style={{ display:"flex", alignItems:"center", gap:5, fontSize:11, color:"rgba(20,45,70,0.5)" }}>
                          <Monitor style={{ width:11, height:11, marginTop:1 }}/>
                          <span>outlook.live.com ou app local</span>
                        </div>
                        <div style={{ marginTop:8, padding:"6px 10px", borderRadius:8, background:"rgba(0,120,212,0.06)", border:"1px solid rgba(0,120,212,0.12)", fontSize:10, color:"#0078d4", fontWeight:600 }}>
                          Recomendado para contas Microsoft 365
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Dica */}
                    {emailProvider && (
                      <motion.div initial={{ opacity:0, y:4 }} animate={{ opacity:1, y:0 }} style={{ marginTop:12, padding:"10px 14px", borderRadius:10, background:"rgba(41,128,185,0.06)", border:"1px solid rgba(41,128,185,0.15)", display:"flex", gap:8, alignItems:"flex-start" }}>
                        <Info style={{ width:14, height:14, color:"#2980b9", flexShrink:0, marginTop:1 }}/>
                        <div style={{ fontSize:11, color:"rgba(20,45,70,0.6)", lineHeight:1.5 }}>
                          {emailProvider === "gmail"
                            ? "Ao clicar em \"Enviar e-mail\" em qualquer contato, o Gmail será aberto em uma nova aba com o destinatário preenchido."
                            : "Ao clicar em \"Enviar e-mail\", o Outlook será aberto via protocolo mailto: — funciona tanto com o app instalado quanto pelo navegador."}
                        </div>
                      </motion.div>
                    )}
                  </motion.div>
                  ...
                  <motion.div className="glass-card" style={{ padding:"16px 20px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:16 }} initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.15 }}>
                    <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
                      <div style={{ fontSize:12, color:"rgba(20,45,70,0.5)" }}>
                        ...
                        {emailProvider === "gmail" ? "Gmail" : emailProvider === "outlook" ? "Outlook" : <span style={{ color:"rgba(20,45,70,0.35)" }}>não configurado</span>}
                      </div>
                      <div style={{ fontSize:12, color:"rgba(20,45,70,0.5)" }}>
