...
export default function GoogleCallback() {
    ...
    fetch(`${API}/auth/google/callback?code=${encodeURIComponent(code)}`, {
      ...
      .then(data => {
        if (data.msg) {
          ...
          setStatus("error");
          setMsg(data.detail || "Erro ao conectar Google.");
          setTimeout(() => navigate("/calendario"), 3000);
        }
      })
      .catch(() => {
  ...
  return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"center", height:"100vh", background:"linear-gradient(145deg,#c8e8f5,#d6eef5,#cceee8)", fontFamily:"sans-serif" }}>
      <div style={{ background:"rgba(255,255,255,0.9)", borderRadius:20, padding:"40px 48px", textAlign:"center", boxShadow:"0 8px 40px rgba(66,133,244,0.15)", maxWidth:360 }}>
        {status === "loading" && (
          <>
            <div style={{ width:48, height:48, border:"4px solid rgba(66,133,244,0.15)", borderTop:"4px solid #4285F4", borderRadius:"50%", margin:"0 auto 20px", animation:"spin 0.8s linear infinite" }} />
            <div style={{ fontSize:16, fontWeight:700, color:"#0f2133" }}>Conectando Google...</div>
