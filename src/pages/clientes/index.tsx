...
import {
  ...
  Thermometer, TrendingUp, Eye, Edit3, Trash2,
...
const MOCK: Empresa[] = [
  ...
  { empresa_id:"3", nome:"GlobalTech",            segmento:"Serviços",    porte:"Grande", cidade:"Rio de Janeiro",status:"Lead",       temperatura:"Frio",   ticket_medio_estimado:null,  responsavel_principal:"João Mendes",    origem_lead:"Prospecção ativa",ultima_interacao:"2025-05-30", proxima_acao:"Qualificar",        contatos_count:1 },
  { empresa_id:"4", nome:"MarketPro",             segmento:"Marketing",   porte:"Pequeno",cidade:"São Paulo",    status:"Fechado",    temperatura:"Quente", ticket_medio_estimado:4200,  responsavel_principal:"Carla Rocha",    origem_lead:"Site",           ultima_interacao:"2025-06-12", proxima_acao:"Onboarding",        contatos_count:4 },
  { empresa_id:"5", nome:"StartGrowth",           segmento:"Financeiro",  porte:"Médio",  cidade:"Curitiba",     status:"Em contato", temperatura:"Morno",  ticket_medio_estimado:11000, responsavel_principal:"Felipe Costa",   origem_lead:"Evento",         ultima_interacao:"2025-06-07", proxima_acao:"Reunião",           contatos_count:2 },
  { empresa_id:"6", nome:"GreenTech",             segmento:"Agronegócio", porte:"Grande", cidade:"Goiânia",      status:"Proposta",   temperatura:"Quente", ticket_medio_estimado:22000, responsavel_principal:"Mariana Silva",  origem_lead:"LinkedIn",       ultima_interacao:"2025-06-11", proxima_acao:"Negociar contrato", contatos_count:5 },
  { empresa_id:"7", nome:"DataBridge",            segmento:"Tecnologia",  porte:"Médio",  cidade:"Belo Horizonte",status:"Lead",      temperatura:"Frio",   ticket_medio_estimado:6000,  responsavel_principal:"Ricardo Nunes",  origem_lead:"Email",          ultima_interacao:"2025-05-20", proxima_acao:"Primeiro contato",  contatos_count:1 },
  { empresa_id:"8", nome:"Construtora Apex",      segmento:"Construção",  porte:"Grande", cidade:"São Paulo",    status:"Fechado",    temperatura:"Quente", ticket_medio_estimado:45000, responsavel_principal:"Sônia Barros",   origem_lead:"Indicação",      ultima_interacao:"2025-06-09", proxima_acao:"Renovação",         contatos_count:3 },
];
...
export default function TodosClientes() {
  ...
  const handleRowClick = (id: string) => {
    ...
    if (now - last < 400) {
      // double click → individual dashboard
  ...
  const SortTh = ({ label, field }: { label: string; field: keyof Empresa }) => (
    <button
      ...
      style={{
        ...
        cursor:"pointer", fontSize:11, fontWeight:700, letterSpacing:"0.07em",
        textTransform:"uppercase", color:"rgba(20,45,70,0.5)",
      }}
    >
      {label}
      ...
      <div style={{ flex:1, height:"100vh", overflowY:"auto", position:"relative", zIndex:5 }}>
        ...
        <div style={{ position:"sticky", top:0, zIndex:20, padding:"14px 28px", background:"rgba(210,238,248,0.75)", backdropFilter:"blur(20px)", borderBottom:"1px solid rgba(255,255,255,0.6)", display:"flex", alignItems:"center", gap:14 }}>
          <div style={{ flex:1 }}>
            ...
            <p style={{ fontSize:12, color:"rgba(20,45,70,0.5)", marginTop:1 }}>
              {filtered.length} empresa{filtered.length!==1?"s":""} encontrada{filtered.length!==1?"s":""} · <span style={{ color:"rgba(20,45,70,0.35)" }}>Duplo clique para abrir o perfil</span>
            </p>
          </div>
        ...
        <div style={{ padding:"22px 28px 40px", display:"flex", flexDirection:"column", gap:18 }}>
                ...
                {filtered.map((emp, idx) => {
                  ...
                  return (
                    <motion.div
                      ...
                      title="Duplo clique para abrir o perfil"
                    >
                      {/* Nome + avatar */}
                      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                      ...
                      {/* Status */}
                      <span className="chip" style={{ background:sc.bg, color:sc.text, border:`1px solid ${sc.border}` }}>
                        {emp.status || "—"}
                      </span>

                      {/* Temperatura */}
                      <span className="chip" style={{ background:tc.bg, color:tc.text }}>
                      ...
                      <div style={{ display:"flex", alignItems:"center", gap:5 }} onClick={e => e.stopPropagation()}>
                        <button
                          ...
                          style={{ background:"rgba(41,128,185,0.08)", color:"#2980b9" }}
                          onClick={e => handleView(e, emp.empresa_id)}
                          title="Ver perfil"
                        >
                        ...
                        <button
                          className="action-btn"
                          style={{ background:"rgba(142,68,173,0.08)", color:"#8e44ad" }}
                          onClick={e => handleEdit(e, emp.empresa_id)}
                          title="Editar"
                        >
                        ...
                        <button
                          className="action-btn"
                          style={{ background: isDeleting ? "rgba(231,76,60,0.2)" : "rgba(231,76,60,0.08)", color:"#e74c3c" }}
                          onClick={e => handleDelete(e, emp.empresa_id)}
          ...
          <p style={{ textAlign:"center", fontSize:11, color:"rgba(20,45,70,0.35)" }}>
            💡 Dê <strong>duplo clique</strong> em uma empresa para abrir o perfil completo
          </p>
        </div>
      </div>
    </div>
