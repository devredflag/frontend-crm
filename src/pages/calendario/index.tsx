...
function tipoInfo(tipo: string) {
  return TIPOS.find(t => t.key === tipo) || TIPOS[4];
}
...
export default function Calendario() {
  ...
  const [form, setForm] = useState({
    titulo:"", tipo:"call", data:"", hora_inicio:"09:00", hora_fim:"10:00",
  ...
  const openEdit = (ev: Evento, e: React.MouseEvent) => {
    ...
    setForm({
      ...
      hora_fim: ev.hora_fim?.slice(0,5)||"10:00",
      empresa_id: ev.empresa_id||"", empresa_nome: ev.empresa_nome||"",
  ...
  const eventosNoDia = (dateStr: string) => eventos.filter(e => e.data === dateStr);
  ...
  const buildMonth = () => {
    ...
    for (let i=0; i<first; i++) {
      ...
      days.push({y:prev.getFullYear(),m:prev.getMonth(),d:prev.getDate(),cur:false});
    }
    for (let d=1; d<=last; d++) days.push({y,m,d,cur:true});
    while (days.length%7!==0) {
      const next = new Date(y, m+1, days.length-first-last+1);
      days.push({y:next.getFullYear(),m:next.getMonth(),d:next.getDate(),cur:false});
    }
    return days;
  };
  ...
  const EventChip = ({ ev, onClick }: { ev:Evento; onClick:(e:React.MouseEvent)=>void }) => {
    ...
    return (
      <div
        onClick={onClick}
        style={{ display:"flex", alignItems:"center", gap:4, padding:"2px 7px", borderRadius:6, background:t.bg, color:t.color, fontSize:10, fontWeight:700, cursor:"pointer", marginBottom:2, overflow:"hidden" }}
      >
      ...
      <div style={{ flex:1, height:"100vh", overflowY:"auto", position:"relative", zIndex:5, display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ position:"sticky", top:0, zIndex:20, padding:"14px 28px", background:"rgba(210,238,248,0.85)", backdropFilter:"blur(20px)", borderBottom:"1px solid rgba(255,255,255,0.6)", display:"flex", alignItems:"center", gap:14, flexShrink:0 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            ...
            <button onClick={next} style={{ width:32, height:32, borderRadius:8, border:"1px solid rgba(200,225,240,0.9)", background:"rgba(255,255,255,0.75)", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
              <ChevronRight style={{ width:15, height:15, color:"#2980b9" }} />
            </button>
            <button onClick={() => setCurrentDate(new Date())} style={{ padding:"6px 12px", borderRadius:8, border:"1px solid rgba(41,128,185,0.3)", background:"rgba(41,128,185,0.08)", color:"#2980b9", fontSize:12, fontWeight:600, cursor:"pointer" }}>
              Hoje
            </button>
          </div>
          ...
          <button
            onClick={() => openNew(toDateStr(today.getFullYear(),today.getMonth(),today.getDate()))}
            style={{ height:38, padding:"0 16px", borderRadius:10, border:"none", cursor:"pointer", background:"linear-gradient(135deg,#2980b9,#1abc9c,#2ecc71,#2980b9)", backgroundSize:"200% 200%", animation:"gradientShift 4s ease infinite", color:"#fff", fontSize:13, fontWeight:700, display:"flex", alignItems:"center", gap:6 }}
          >
                  ...
                  {buildMonth().map((day, i) => {
                    ...
                    return (
                      <div key={i} className={`day-cell${!day.cur?" other-month":""}${tod?" today":""}`} onClick={() => openNew(dateStr)}>
                        <div style={{ display:"flex", alignItems:"center", justifyContent:"center", width:26, height:26, borderRadius:"50%", background: tod ? "#2980b9" : "transparent", color: tod ? "#fff" : day.cur ? "#0f2133" : "rgba(20,45,70,0.4)", fontSize:13, fontWeight:700, marginBottom:4 }}>
                          {day.d}
                        </div>
                        {evs.slice(0,3).map(ev => (
                          <EventChip key={ev.evento_id} ev={ev} onClick={e => openEdit(ev, e)} />
                        ))}
                  ...
                  {buildWeek().map((d, i) => {
                    ...
                    return (
                      <div key={i} style={{ padding:"10px 0", textAlign:"center", borderLeft:"1px solid rgba(200,225,240,0.4)" }}>
                        ...
                        <div style={{ width:28, height:28, borderRadius:"50%", background: tod ? "#2980b9" : "transparent", color: tod ? "#fff" : "#0f2133", fontSize:14, fontWeight:800, display:"flex", alignItems:"center", justifyContent:"center", margin:"4px auto 0" }}>
                          {d.getDate()}
                        </div>
                      </div>
                    );
                ...
                <div style={{ flex:1, overflow:"auto" }}>
                  {HORAS.map(hora => (
                    <div key={hora} style={{ display:"grid", gridTemplateColumns:"60px repeat(7,1fr)", minHeight:56 }}>
                      <div style={{ padding:"6px 8px 0", fontSize:11, fontWeight:600, color:"rgba(20,45,70,0.4)", textAlign:"right", borderRight:"1px solid rgba(200,225,240,0.4)", flexShrink:0 }}>
                        {padZero(hora)}:00
                      </div>
                      {buildWeek().map((d, i) => {
              ...
              <div style={{ flex:1, overflow:"auto", display:"flex", flexDirection:"column" }}>
                <div style={{ padding:"14px 20px", borderBottom:"1px solid rgba(200,225,240,0.5)", display:"flex", alignItems:"center", gap:12, flexShrink:0 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:11, color:"rgba(20,45,70,0.45)" }}>
                      {eventosNoDia(toDateStr(currentDate.getFullYear(),currentDate.getMonth(),currentDate.getDate())).length} evento(s)
                    </div>
                  </div>
                </div>
                <div style={{ flex:1, overflow:"auto" }}>
                  {HORAS.map(hora => {
                    ...
                    return (
                      <div key={hora} style={{ display:"flex", minHeight:72, borderBottom:"1px solid rgba(200,225,240,0.3)", background: isNow ? "rgba(41,128,185,0.03)" : "transparent" }} onClick={() => openNew(dateStr, `${padZero(hora)}:00`)}>
                        <div style={{ width:72, padding:"10px 12px 0", fontSize:12, fontWeight:600, color: isNow ? "#2980b9" : "rgba(20,45,70,0.4)", textAlign:"right", flexShrink:0, borderRight:"2px solid " + (isNow ? "#2980b9" : "rgba(200,225,240,0.4)") }}>
                          {padZero(hora)}:00
                        </div>
                          ...
                          {evs.map(ev => {
                            ...
                            return (
                              <motion.div
                                ...
                                style={{ padding:"8px 14px", borderRadius:10, background:t.bg, border:`1.5px solid ${t.color}30`, display:"flex", alignItems:"center", gap:10, cursor:"pointer" }}
                              >
                                <div style={{ width:28, height:28, borderRadius:8, background:t.color, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                                  <t.icon style={{ width:13, height:13, color:"#fff" }} />
                                </div>
                                <div style={{ flex:1 }}>
          ...
          <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }} style={{ position:"fixed", inset:0, zIndex:100, background:"rgba(10,30,50,0.45)", backdropFilter:"blur(4px)", display:"flex", alignItems:"center", justifyContent:"center", padding:20 }} onClick={() => setShowModal(false)}>
            <motion.div initial={{ opacity:0, scale:0.94, y:20 }} animate={{ opacity:1, scale:1, y:0 }} exit={{ opacity:0, scale:0.94 }} transition={{ duration:0.22 }} onClick={e => e.stopPropagation()} style={{ width:"100%", maxWidth:540, borderRadius:20, background:"rgba(230,245,252,0.97)", backdropFilter:"blur(24px)", border:"1px solid rgba(255,255,255,0.9)", boxShadow:"0 24px 80px rgba(41,128,185,0.2)", padding:28, maxHeight:"90vh", overflowY:"auto" }}>
              ...
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
                <div style={{ fontSize:16, fontWeight:800, color:"#0f2133" }}>
                  {editEvento ? "Editar evento" : "Novo evento"}
                </div>
                <div style={{ display:"flex", gap:8 }}>
                  {editEvento && (
                    <button onClick={() => handleDelete(editEvento.evento_id)} style={{ width:34, height:34, borderRadius:8, border:"1px solid rgba(231,76,60,0.3)", background:"rgba(231,76,60,0.08)", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer" }}>
                      <Trash2 style={{ width:14, height:14, color:"#e74c3c" }} />
                    </button>
                  )}
                  <button onClick={() => setShowModal(false)} style={{ width:34, height:34, borderRadius:8, border:"1px solid rgba(200,225,240,0.9)", background:"rgba(255,255,255,0.7)", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer" }}>
