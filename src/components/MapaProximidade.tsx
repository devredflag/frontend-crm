...
export default function MapaProximidade({
  ...
  useEffect(() => {
    ...
    pontos.forEach((p, i) => {
      ...
      dists.forEach((d) => {
        ...
        L.polyline([[p.lat, p.lng], [d.q.lat, d.q.lng]], { color:"#9FD3EA", weight: 1.5, opacity: opac, dashArray: "4 4" }).addTo(group);
      });
    });
    ...
    pontos.forEach((p) => {
      ...
      const popup = `
        <div style="font-family:'Plus Jakarta Sans',sans-serif;min-width:190px">
          <div style="font-size:13px;font-weight:800;color:#EAF6FB">${escapeHtml(p.emp.nome)}</div>
          <div style="font-size:11px;color:#7a8a9a;margin-top:2px">${escapeHtml(p.emp.segmento || "—")}${p.emp.cidade ? " · " + escapeHtml(p.emp.cidade) : ""}</div>
          ${listaVz}
          ${vz.length ? '<div style="margin-top:6px;font-size:11px;color:#9FD3EA;font-weight:700">Clique no ponto para ver a rota pelas ruas</div>' : ""}
        </div>`;
  ...
  if (erro) {
    return (
      <div style={{ padding: 40, textAlign: "center", fontSize: 14, color:"#9FD3EA", fontWeight: 600 }}>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 12, fontWeight: 700, color:"#EAF6FB" }}>Raio de proximidade</span>
          <input type="range" min={1} max={50} step={1} value={raioKm} onChange={(e) => setRaioKm(Number(e.target.value))} style={{ accentColor: "#9FD3EA", width: 160 }} />
          <span style={{ fontSize: 12, fontWeight: 800, color:"#9FD3EA", minWidth: 48 }}>{raioKm} km</span>
        </div>

        <div style={{ display: "flex", alignItems: "center", gap: 10, marginLeft: "auto", flexWrap: "wrap" }}>
          {[{ l: "Quente", c: "#F7B8B1" }, { l: "Morno", c: "#F2C879" }, { l: "Frio", c: "#9FD3EA" }, { l: "Sem temp.", c: "#7f8c9a" }].map((s) => (
            <span key={s.l} style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 11, fontWeight: 600, color:"#9FD3EA" }}>
              <span style={{ width: 10, height: 10, borderRadius: "50%", background:s.c, border:"1.5px solid rgba(159,211,234,0.18)", boxShadow: "0 0 0 1px rgba(0,0,0,0.08)" }} />
              {s.l}
            </span>
          ))}
        </div>
      ...
      {semCoords > 0 && onGeocodificar && (
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", borderRadius: 10, background:"rgba(230,126,34,0.08)", border:"1.5px solid rgba(230,126,34,0.25)", flexWrap: "wrap" }}>
          <span style={{ fontSize: 12, fontWeight: 600, color:"#F2C879", flex: 1, minWidth: 200 }}>
            {gc?.rodando
          ...
          <button
            ...
            disabled={gc?.rodando}
            style={{ height: 32, padding: "0 14px", borderRadius: 8, border:"none", cursor: gc?.rodando ? "wait" : "pointer", background:"linear-gradient(135deg,#F2C879,#F2C879)", color:"#EAF6FB", fontSize: 12, fontWeight: 700, opacity: gc?.rodando ? 0.7 : 1, whiteSpace: "nowrap" }}
          >
      ...
      {rotaSel && (
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", borderRadius: 10, background:"rgba(142,68,173,0.07)", border:"1.5px solid rgba(142,68,173,0.22)", flexWrap: "wrap" }}>
          <span style={{ fontSize: 12, color:"#5a3d6b", flex: 1, minWidth: 220 }}>
            {rotando
              ...
              : rotaSel.destinos.length
                ? <><b style={{ color:"#9FD3EA" }}>Rotas de {rotaSel.origem}:</b>{" "}
          ...
          <button onClick={limparRotas} style={{ height: 28, padding: "0 12px", borderRadius: 8, border:"1px solid rgba(142,68,173,0.3)", background:"rgba(142,68,173,0.06)", color:"#9FD3EA", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
            Limpar rota
          </button>
        </div>
      )}
      ...
      {/* Mapa */}
      <div style={{ position: "relative", borderRadius: 14, overflow: "hidden", border:"1px solid rgba(159,211,234,0.18)" }}>
        {!ready && (
          <div style={{ position: "absolute", inset: 0, zIndex: 5, display: "flex", alignItems: "center", justifyContent: "center", background:"rgba(159,211,234,0.08)", fontSize: 13, fontWeight: 600, color:"#9FD3EA" }}>
            Carregando mapa...
          </div>
        )}
      ...
      {/* Rodapé informativo */}
      <div style={{ fontSize: 11, color:"#9FD3EA", display: "flex", gap: 14, flexWrap: "wrap" }}>
        <span><b style={{ color:"#9FD3EA" }}>{pontos.length}</b> empresa{pontos.length !== 1 ? "s" : ""} no mapa</span>
        {semCoords > 0 && <span style={{ color:"#F2C879" }}>{semCoords} sem localização</span>}
