...
export default function VendasInsights() {
  ...
  const cards = [
    { label: "Orçamentos",     value: String(d.total_orcamentos),   icon: FileText,   color: "#2563EB" },
    ...
    { label: "Conversão",      value: `${d.taxa_conversao}%`,       icon: Percent,    color: "#5B6570" },
  ];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(170px,1fr))", gap: 12 }}>
        {cards.map(c => (
          <div key={c.label} style={{ background: "#ffffff", border: "1px solid #ffffff", borderRadius: 8, padding: "16px 18px", display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ width: 40, height: 40, borderRadius: 8, background: `${c.color}1a`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <c.icon style={{ width: 18, height: 18, color: c.color }} />
            </div>
            <div style={{ minWidth: 0 }}>
              <div style={{ fontSize: 9, fontWeight: 700, color: "#5B6570", textTransform: "uppercase", letterSpacing: "0.07em" }}>{c.label}</div>
              <div style={{ fontSize: 18, fontWeight: 900, color: "#16191D", letterSpacing: "-0.02em" }}>{c.value}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(300px,1fr))", gap: 12 }}>
        ...
        <div style={{ background: "#ffffff", border: "1px solid #ffffff", borderRadius: 8, padding: "18px 20px" }}>
          <div style={{ fontSize: 13, fontWeight: 900, color: "#16191D", marginBottom: 14 }}>Orçamentos por status</div>
              ...
              return (
                <div key={s}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                    ...
                    <span style={{ fontSize: 11, fontWeight: 800, color: "#16191D" }}>{v.total}</span>
                    <span style={{ fontSize: 10, color: "#5B6570", minWidth: 90, textAlign: "right" }}>{brl(v.valor)}</span>
                  </div>
                  <div style={{ height: 6, borderRadius: 4, background: "#E3E6E9", overflow: "hidden" }}>
        ...
        <div style={{ background: "#ffffff", border: "1px solid #ffffff", borderRadius: 8, padding: "18px 20px" }}>
          <div style={{ fontSize: 13, fontWeight: 900, color: "#16191D", marginBottom: 14 }}>Equipamentos mais orçados</div>
          {d.equipamentos_mais_orcados.length === 0 ? (
            <div style={{ padding: "30px 0", textAlign: "center", color: "#5B6570" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
              {d.equipamentos_mais_orcados.map(e => (
                <div key={e.nome}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                    <span style={{ fontSize: 11, fontWeight: 700, color: "#16191D", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{e.nome}</span>
                    <span style={{ fontSize: 11, fontWeight: 800, color: "#2563EB" }}>{e.quantidade}x</span>
                    <span style={{ fontSize: 10, color: "#5B6570", minWidth: 90, textAlign: "right" }}>{brl(e.valor)}</span>
                  </div>
                  <div style={{ height: 6, borderRadius: 4, background: "#E3E6E9", overflow: "hidden" }}>
                    <div style={{ height: "100%", width: `${(e.quantidade / maxQtd) * 100}%`, background: "#2563EB", borderRadius: 4 }} />
                  </div>
                </div>
              ))}
            </div>
