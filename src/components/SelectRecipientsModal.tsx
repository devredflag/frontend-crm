...
const CHANNEL_CONFIG = {
  email:    { label:"E-mail",    color:"#9FD3EA", bg:"rgba(159,211,234,0.55)",  border:"rgba(159,211,234,0.30)",  icon:Mail          },
  whatsapp: { label:"WhatsApp",  color:"#83DDA8", bg:"rgba(39,174,96,0.08)",   border:"rgba(39,174,96,0.25)",   icon:MessageCircle },
  telefone: { label:"Telefone",  color:"#F2C879", bg:"rgba(230,126,34,0.08)",  border:"rgba(230,126,34,0.25)",  icon:Phone         },
          ...
          >
            ...
            >
              ...
              <button onClick={onClose} style={{
                ...
                color:"#9FD3EA", zIndex:2, transition:"all 0.18s",
              }}
                onMouseEnter={e => (e.currentTarget.style.color="#F7B8B1")}
                onMouseLeave={e => (e.currentTarget.style.color="rgba(159,211,234,0.55)")}
              >
              ...
              <div style={{ padding:"24px 24px 20px", overflowY:"auto", maxHeight:"calc(90vh - 4px)" }}>
                ...
                {channel === "email" && checkingAuth && (
                  <div style={{ padding:"40px 0", textAlign:"center" }}>
                    <div style={{ width:36, height:36, border:`3px solid rgba(159,211,234,0.30)`, borderTop:`3px solid rgba(159,211,234,0.30)`, borderRadius:"50%", margin:"0 auto 16px", animation:"spin 0.8s linear infinite" }}/>
                    <div style={{ fontSize:13, fontWeight:600, color:"#9FD3EA" }}>Verificando provedores...</div>
                ...
                <AnimatePresence mode="wait">
                  {!checkingAuth && step === "provider" && (
                    ...
                    >
                      ...
                      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:22 }}>
                        ...
                        <div>
                          <div style={{ fontSize:16, fontWeight:800, color:"#EAF6FB", letterSpacing:"-0.02em" }}>
                            Enviar e-mail
                          </div>
                          <div style={{ fontSize:12, color:"#9FD3EA" }}>
                            {provider
                              ? <>Padrão: <strong style={{ color:"#EAF6FB" }}>{provider === "gmail" ? "Gmail" : "Outlook"}</strong> — clique para confirmar ou trocar</>
                  ...
                  {!checkingAuth && step === "recipients" && (
                    ...
                    >
                      ...
                      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:20 }}>
                        ...
                        {channel === "email" && outlookOk && gmailOk && (
                          <button onClick={() => setStep("provider")} style={{
                            ...
                            onMouseLeave={e => (e.currentTarget.style.background="rgba(18,59,94,0.55)")}
                          >
                            <ChevronLeft style={{ width:14, height:14, color:"#9FD3EA" }}/>
                          </button>
                        )}
                            ...
                            {recipients.length === 0 ? <EmptyState cfg={cfg}/> : (
                              <>
                                ...
                                <button onClick={toggleAll} style={{
                                  ...
                                  border:`1.5px solid ${allSelected ? cfg.border:"rgba(159,211,234,0.18)"}`,
                                  background:allSelected ? cfg.bg : "rgba(18,59,94,0.55)",
                                  cursor:"pointer", transition:"all 0.18s",
                                }}>
                                  <Checkbox checked={allSelected} color={cfg.color}/>
                                  <span style={{ fontSize:12, fontWeight:700, color:allSelected ? cfg.color:"#EAF6FB" }}>
                      ...
                      {/* Divisor + botões */}
                      <div style={{ height:1, background:"rgba(159,211,234,0.08)", margin:"18px 0 16px" }}/>
                      <div style={{ display:"flex", gap:10 }}>
                        <button onClick={onClose} style={{
                          ...
                          fontWeight:600, fontSize:13, color:"#EAF6FB",
                        }}>Cancelar</button>
...
function ProviderButton({ name, subtitle, color, bgColor, borderColor, hoverBg, hoverBorder, icon, selected, onClick }: {
  ...
  return (
    ...
    >
      ...
      <div style={{ flex:1 }}>
        <div style={{ fontSize:14, fontWeight:700, color:"#EAF6FB" }}>{name}</div>
        <div style={{ fontSize:11, color:"#9FD3EA", marginTop:2 }}>{subtitle}</div>
      </div>
      <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:4, flexShrink:0 }}>
        ...
        {selected && (
          <div style={{ fontSize:9, fontWeight:700, padding:"2px 8px", borderRadius:6, background:color, color:"#EAF6FB" }}>
            ✓ Padrão
          </div>
        )}
      </div>
...
function OutlookIcon({ size = 22 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 22 22" fill="none">
      ...
      <path d="M1 8l10 6 10-6" stroke="#9FD3EA" strokeWidth="1.5"/>
    </svg>
  );
}
...
function Checkbox({ checked, color }: { checked: boolean; color:string }) {
  return (
    <div style={{
      ...
      border:`1.5px solid ${checked ? color:"#EAF6FB"}`,
      background:checked ? color:"#EAF6FB",
      display:"flex", alignItems:"center", justifyContent:"center",
    }}>
      {checked && (
...
function RecipientRow({ recipient, cfg, checked, onToggle, hideCheckbox }: {
  ...
  return (
    <div onClick={onToggle} style={{
      ...
      border:`1.5px solid ${checked && !hideCheckbox ? cfg.border:"rgba(159,211,234,0.18)"}`,
      background:checked && !hideCheckbox ? cfg.bg : "rgba(18,59,94,0.55)",
      cursor:hideCheckbox ? "default" : "pointer",
    }}>
      {!hideCheckbox && <Checkbox checked={checked} color={cfg.color}/>}
      <div style={{ width:32, height:32, borderRadius:"50%", background:avatarColor(recipient.nome), display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, fontWeight:700, color:"#EAF6FB", flexShrink:0 }}>
        {initials(recipient.nome)}
      </div>
      <div style={{ flex:1, minWidth:0 }}>
        <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:2 }}>
          <span style={{ fontSize:12, fontWeight:700, color:"#EAF6FB", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{recipient.nome}</span>
          {recipient.principal && (
          ...
          {recipient.decisor && (
            <span style={{ fontSize:9, fontWeight:700, padding:"1px 6px", borderRadius:4, background:"rgba(39,174,96,0.1)", color:"#83DDA8", border:"1px solid rgba(39,174,96,0.2)", flexShrink:0 }}>Decisor</span>
          )}
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:5, fontSize:11, color:cfg.color, fontWeight:500 }}>
          ...
          <span style={{ whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>
            {recipient.valor || <span style={{ color:"#9FD3EA", fontStyle:"italic" }}>Não informado</span>}
          </span>
        </div>
      </div>
      {recipient.funcao && (
        <span style={{ fontSize:10, color:"#9FD3EA", whiteSpace:"nowrap", flexShrink:0 }}>{recipient.funcao}</span>
      )}
    </div>
  );
}
...
function ModeButton({ active, color, icon, label, sub, onClick }: {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      border:`1.5px solid ${active ? color+"50" : "rgba(159,211,234,0.18)"}`,
      background:active ? `${color}0d` : "rgba(18,59,94,0.55)",
      cursor:"pointer",
      boxShadow:active ? `0 0 0 3px ${color}15` : "none",
    }}>
      <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:3 }}>
        <span style={{ color:active ? color:"#9FD3EA" }}>{icon}</span>
        <span style={{ fontSize:12, fontWeight:700, color:active ? color:"#9FD3EA" }}>{label}</span>
      </div>
      <div style={{ fontSize:10, color:active ? color+"aa" : "#9FD3EA", fontWeight:500, paddingLeft:21 }}>{sub}</div>
    </button>
  );
}
...
function EmptyState({ cfg }: { cfg: typeof CHANNEL_CONFIG[SendChannel] }) {
  return (
    <div style={{ padding:"28px 16px", textAlign:"center", borderRadius:12, border:"1.5px dashed rgba(159,211,234,0.18)", background:"rgba(159,211,234,0.08)" }}>
      <cfg.icon style={{ width:28, height:28, color:`${cfg.color}40`, margin:"0 auto 8px" }}/>
      <div style={{ fontSize:12, fontWeight:600, color:"#9FD3EA" }}>
